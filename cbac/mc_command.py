...
class MCCommand(object):
    ...
    """
    def __init__(self, is_conditional=False, is_creates_condition=False):
        """
        ...
        :param is_creates_condition: If the command creates condition for other commands. Meaning this command will
        force the next command inline to be conditional.
        This is mainly used for testforblock commands.
        """
        self.is_conditional = is_conditional
        self.is_creates_condition = is_creates_condition
...
class SimpleCommand(MCCommand):
    ...
    """
    def __init__(self, body, is_conditional=False, is_creates_condition=False):
        super(SimpleCommand, self).__init__(is_conditional, is_creates_condition)
        self._body = body
...
class LazyCommand(MCCommand):
    ...
    """
    def __init__(self, func, is_conditional=False, is_creates_condition=False, *args, **kwargs):
        assert all([isinstance(item, bool) for item in [is_conditional, is_creates_condition]]), "must be bool"
        super(LazyCommand, self).__init__(is_conditional, is_creates_condition)
        self.args = args
        self.kwargs = kwargs
        self.func = func
