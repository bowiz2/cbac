...
class TestEntity(TestCase):
    def test_ctor(self):
        ent = MCEntity(MCEntity.ID.ARMOR_STAND, custom_name="a", no_gravity=True)
        self.assertEqual(ent.custom_name, "a")
        self.assertEqual(ent.no_gravity, 1)

        ent = MCEntity(MCEntity.ID.ARMOR_STAND, custom_name="b", no_gravity=False)
    ...
    def test_uuid_name(self):
        ...
        :return:
        """
        ent1 = MCEntity(MCEntity.ID.ARMOR_STAND, no_gravity=True)
        ent2 = MCEntity(MCEntity.ID.ARMOR_STAND, no_gravity=True)
    ...
    def test_parsing(self):
        entity = MCEntity(MCEntity.ID.ARMOR_STAND, custom_name="p_pivot", no_gravity=True)
